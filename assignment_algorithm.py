...
if __name__ == '__main__':
    ...
    for i in range(1, 9999):
        ...
        output_assignments, size, steps, cache_hits = IncrementalAssignmentAlgorithm(
            ...
            # Couldn't load tuples from the JSON file but I am too lazy to use arrays instead,
            # so im just casting all the solution arrays from the json to tuples here
            [(a[0], a[1]) for a in examples[0]['solution']],
            'exhaustive'
        ).run()

        write_to_csv(
            [(str(size), str(steps), str(cache_hits))],
            'exhaustive'
        )
        ...
        for example in examples:
            ...
            history.append(
                (int(steps), int(cache_hits), valid)
            )
            write_to_csv(
                [(str(size), str(steps), str(cache_hits), str(valid), str(incremental_total), str(munkres_total))],
                'open_delta'
        ...
        write_to_csv([(
            str(size),
            str(sum([val[0] for val in history]) / len(history)),
            str(sum([val[1] for val in history]) / len(history)),
            str(len([True for val in history if val[2]]) / len(history))
        )], 'open_delta_avg')
